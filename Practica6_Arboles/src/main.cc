...
void Usage(char* argv[]) {
  std::cout << "Modo de uso: " << argv[0] << " [-h | --help ] -ab <'abb'|'abe'> -init <'manual'|'random' <nº elementos a generar> |'file' <nº elementos a generar> <nombre del fichero>>" << std::endl << std::endl;
  ...
  for (int i = 1; i < argc; i++) {
    ...
    // Opción -init: Selecciona el método de inicialización de los nodos del árbol
      ...
      if (std::string(argv[i + 1]) == "manual") {
        ...
        std::cout << "Generando " << num_elements << " claves aleatorias para insertar en el árbol..." << std::endl;
        ...
        std::cout << "Leyendo " << num_elements << " claves del fichero " << filename << " para insertar en el árbol..." << std::endl;
        for (int i = 0; i < num_elements; i++) {
          input_file >> key;
